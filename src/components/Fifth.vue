...
export default {
  ...
  data () {
    return {
      ...
      setPrice: '',
      istrue: '',
      computerNum: localStorage.getItem('videoPrice') || 5,
      pricetimer: null
    }
  },
  mounted () {
  ...
  methods: {
    saveForm () {
      var data = {
        ...
        updating: {
          participantMaxPage: 16,
          participantCurrentAppName: 'Video Watching',
          ParticipantCurrentPageName: 'Video Watching-introduction',
          finishedOrNo: false
        },
        taboo: {},
        dotCounting: {},
        snake: {},
        video: {
          computerPrice: localStorage.getItem('videoPrice'),
    ...
    getPrice () {
      localStorage.setItem('videoUserPrice', this.setPrice)
      ...
      this.saveForm()
      this.$router.push({name: 'QuizResults', query: {step: 'Fifth', nextstep: 'fifthplay', nextGame: 'sixth', result: this.istrue, setPrice: this.setPrice, computerNum: this.computerNum}})
    }
  }
}
</script>
